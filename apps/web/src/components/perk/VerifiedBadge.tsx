...
const config: Record<Status, { label: string; className: string; Icon: React.ComponentType<{ className?: string }> }> = {
  verified: { label: "Verified", className: "bg-success/10 text-success border-success/20", Icon: CheckCircle2 },
  pending: { label: "Pending review", className: "bg-warning/10 text-warning-foreground border-warning/30", Icon: Clock },
...
export function VerifiedBadge({ status, className }: { status: Status; className?: string }) {
  ...
  return (
    <span className={cn("inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-xs font-medium", variant, className)}>
